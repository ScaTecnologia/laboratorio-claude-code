...
1. Validar localmente: `docker compose up --build -d`, testar login/clientes/fornecedores em `http://localhost:3000`, e rodar o mesmo scan do CI: `docker run --rm -v /var/run/docker.sock:/var/run/docker.sock aquasec/trivy:0.54.1 image --severity CRITICAL,HIGH --ignore-unfixed --exit-code 1 <imagem>`.
